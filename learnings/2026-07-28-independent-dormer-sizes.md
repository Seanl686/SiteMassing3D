...
- Dormer size inputs step by `0.25` ft (3 in) with a floor of 0.25 ft — the old `step="0.5" min="2"` made it impossible to tune an inner gable small enough to match a photo. A 1-inch step (`0.0833`) was tried first and rejected: it never lands on a whole foot, so the spinner walks 4.0833, 4.1666, … Quarter feet divide evenly into 1 ft. Dormer drag and nest offset snap to the same quarter (`roundQuarter`) so typed and dragged values agree.
